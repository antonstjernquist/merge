...
- **Persistent agent IDs** - Client-controlled IDs that survive restarts
- **Daemon mode** - Self-registering daemon that auto-executes tasks via Claude Code
...
| `merge daemon` | Run as auto-executing daemon (self-registers on startup) |
...
Run a worker that auto-accepts and executes tasks via Claude Code:

```bash
# Start daemon (self-registers with server)
merge daemon \
  --name "Worker" \
  --server http://your-vps:3000 \
  --token your-token \
...
# Or if already joined previously (config is persisted)
...
token: your-token
agentId: <auto-generated-persistent-uuid>
...
**Note:** The `agentId` is auto-generated on first run and persists across restarts. This ensures the same agent can reconnect without server-side state.
